...
// threads that are processing the image
sem_t mutex_write;
// Barrier used to synchonize the thread that is writting the image and the
// threads processing the image (also to synchronize the processing thread
// bethween them)
...
int main(int argc, char *argv[]) {
	...
	if (strcmp(argv[1], "encode") == 0) {
		...
		// Initialize the semaphore and the barrier
		sem_init(&mutex_read, 0, 0);
		sem_init(&mutex_write, 0, 0);
		// We use P - 1 to syncrhonize the encoding threads and the writing thread
		if (pthread_barrier_init(&barrier_wait_encode, NULL, P - 2) != 0) {
...
// Thread function for encode used by the thread that reads the input image
void* threadRead(void *var) {
...
// Thread function for writing the output image, used by encode
void* threadWrite(void *var) {
	int thread_id = *(int*)var;
	int i,j;
...
void* threadEncode(void *var) {
	...
	start_line = ceil((double)nr_lines_needed / (P - 2)) * thread_id;
	end_line = fmin((thread_id + 1) * ceil((double)nr_lines_needed / (P - 2)), nr_lines_needed);
	...
	// Wait for each thread to finish encoding the first part
	...
	// Wait for each thread to finish encoding the second part
	pthread_barrier_wait(&barrier_wait_encode);
